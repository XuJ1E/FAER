...
from utils.loss import Loss, AsymmetricLossOptimized, BCEFocalLoss
...
parser.add_argument('--data_name', default='CelebA', choices=['CelebA', 'LFWA'], help='dataset name of multi-label')
parser.add_argument('--data_path', default='/home/xuujie_ygc/workspace/MM_FOR_ML/data/CelebA/', help='path to dataset')
parser.add_argument('--eval_only', default=False, action='store_true')
parser.add_argument('--eval_ckpt', default='', type=str, help='path to checkpoint for evaluation')
...
parser.add_argument('--arch', default='BASE_MTL_CELEBA', type=str, help='model type for train')
...
def main(args):
    ...
    if args.data_name == 'CelebA':
        ...
        train_dataset = LFWADataset(root=args.data_path,
                                    mode='train',
                                    transform=transforms.Compose([
                                        transforms.Resize((224, 224)),
                                        ...
                                        transforms.ToTensor(),
                                        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]))
        test_dataset = LFWADataset(root=args.data_path,
                                   mode='test',
                                   transform=transforms.Compose([
                                       transforms.Resize((224, 224)),
                                       transforms.ToTensor(),
    ...
    # criterion = AsymmetricLossOptimized().cuda()
